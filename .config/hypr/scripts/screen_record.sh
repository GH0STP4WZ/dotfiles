...
# Requires wl-clipboard for clipboard support
...
startRecording() {
    ...
    if [ "$target" == "screen" ]; then
        ...
        # Check if slurp was cancelled (empty geometry)
        if [ -z "$geometry" ]; then
        ...
        wf-recorder $WF_RECORDER_OPTS --geometry "$geometry" --file "$outputPath" &
    fi
...
stopRecording() {
    ...
    if [ -z "$outputPath" ] || [ ! -f "$outputPath" ]; then
        notify-send "WF Recorder" "Recording stopped. No recent recording found." \
            -i video-x-generic \
            -a "WF Recorder" \
            -t 10000
        exit 1
    ...
    if command -v wl-copy >/dev/null 2>&1; then
        wl-copy --type video/mp4 < "$outputPath" &
        clipboard_status="File copied to clipboard."
    else
        clipboard_status="wl-copy not found. Install wl-clipboard to enable clipboard support."
    fi
...
""|toggle)
    if checkRecording; then
        ...
        # Default behavior if no args: record selected region, save to ~/Videos
        startRecording start region "$HOME/Videos"
    fi
    ;;
start)
    startRecording "$@"
